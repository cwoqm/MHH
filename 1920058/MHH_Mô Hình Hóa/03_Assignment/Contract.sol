...
constact Owner_Staff{
	...
	function createTimestamp1(uint16 year, uint8 month, uint8 day, uint8 hour, uint minute) public{
		timestamp1 = dt.toTimestamp(year,month,day,hour,minute);
	}

	function createTimestamp2(uint16 year, uint8 month, uint8 day, uint8 hour, uint minute) public{
		timestamp2 = dt.toTimestamp(year,month,day,hour,minute);
	}


	function transfer1(uint16 year, uint8 month, uint8 day, uint8 hour, uint minute) public payable{
		if(timestamp1 == dt.toTimestamp(year,month,day,hour,minute)){
			...
			if((timestamp1 < dt.toTimestamp(year,month,day,hour,minute) && (deposit_money>0) && (salary_money==0)){
	...
	function transfer2(uint16 year, uint8 month, uint8 day, uint8 hour, uint minute) public payable{
